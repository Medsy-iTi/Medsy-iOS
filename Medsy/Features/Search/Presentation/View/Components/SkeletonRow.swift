//
//  SkeletonRow.swift
//  Medsy
//
//  Created by Shahudaa on 15/07/2026.
//

import SwiftUI


struct SkeletonRow: View {
    @Environment(\.layoutDirection) private var layoutDirection
    @ObservedObject private var appSettings = AppSettings.shared

    private var isRTL: Bool { layoutDirection == .rightToLeft }

    var body: some View {
        HStack(spacing: MedsySpacing.sm) {

                
                MedsyShimmerBlock(cornerRadius: MedsyRadius.md)
                    .frame(width: 72, height: 72)
                textShimmers
                actionShimmers
           
        }
        .padding(MedsySpacing.sm)
        .frame(height: 96)
        .background(
            RoundedRectangle(cornerRadius: MedsyRadius.lg)
                .fill(AppColor.card)
                .overlay(
                    RoundedRectangle(cornerRadius: MedsyRadius.lg)
                        .stroke(AppColor.border, lineWidth: 1)
                )
        )
    }

    private var actionShimmers: some View {
        VStack(spacing: MedsySpacing.sm) {
            MedsyShimmerBlock(cornerRadius: 10).frame(width: 20, height: 20)
            Spacer(minLength: 0)
            MedsyShimmerBlock(cornerRadius: 13).frame(width: 26, height: 26)
        }
    }

    private var textShimmers: some View {
        VStack(
            alignment: isRTL ? .trailing : .leading,
            spacing: MedsySpacing.xs
        ) {
            MedsyShimmerBlock().frame(width: 120, height: 14)
            MedsyShimmerBlock().frame(width: 80, height: 10)
            Spacer(minLength: MedsySpacing.sm)
            MedsyShimmerBlock().frame(width: 50, height: 12)
        }
        .frame(maxWidth: .infinity, alignment: isRTL ? .trailing : .leading)
    }
}

struct MedsySkeletonList: View {
    var rowCount: Int = 5
    var body: some View {
        VStack(spacing: MedsySpacing.sm) {
            ForEach(0..<rowCount, id: \.self) { _ in
                SkeletonRow()
            }
        }
    }
}
